...
function Exercise1(svgSelection) {
    ...
    this.addReplayStep = function() {
        ...
        replayHistory.push({
            ...
            "s": sCopy,
            "legende": $("#tab_tf1").find(".LegendeText").html(),
    ...
    this.previousStepChoice = function() {        
        ...
        $("#tab_tf1").find(".LegendeText").html(oldState.legende);
    ...
    this.extractMin = function() {
        ...
        if (s.currentNode.id == targetNode.id && questionStats.gestellt < questionStats.numQuestions) {
            this.poseQuestion("assets/question_dijkstra_target_from_pq_"+LNG.getLanguage()+".json");
        };
    };
    ...
    this.loopOutEdges = function () {
        ...
        if (s.pqueue.contains(s.oppositeNode) && (s.neighbourDistance < s.pqueue.getKey(s.oppositeNode))) {
            ...
            if (s.oppositeNode.id == 18 && questionStats.gestellt < questionStats.numQuestions) {
                this.poseQuestion("assets/question_dijkstra_node_s_"+LNG.getLanguage()+".json");
            };
    ...
    this.updateVisitedNode = function() {
        s.pqueue.decreaseKey(Graph.instance.nodes.get(s.oppositeNodeID), s.neighbourDistance);
        ...
        var inEdges = s.oppositeNode.getInEdges();
        for (var i = 0; i < inEdges.length; ++i) {
            var edge = inEdges[i];
            if (edge.id != s.currentEdge.id) {
                s.edgeColors[edge.id] = const_Colors.NormalEdgeColor;
            } else {
                s.edgeColors[s.currentEdge.id] = const_Colors.ShortestPathColor;
            };
        };

        // Knotenlabel updaten
        s.nodeLabels[s.oppositeNodeID] = GraphDrawer.prototype.mapNodeIDToName(parseInt(s.oppositeNode.id)) + ": " + s.neighbourDistance.toString();
    ...
    this.insertNewNode = function() {
        s.pqueue.insert(new Node(s.neighbourDistance, s.oppositeNode));
        s.distance[s.oppositeNodeID] = s.neighbourDistance;
        s.parentEdges[s.oppositeNodeID] = s.currentEdge.id;
        s.parentNodes[s.oppositeNodeID] = s.currentNode.id;
        s.nodeColors[s.oppositeNodeID] = const_Colors.PQColor;
        s.nodeLabels[s.oppositeNodeID] = GraphDrawer.prototype.mapNodeIDToName(parseInt(s.oppositeNode.id)) + ": " + s.distance[s.oppositeNodeID].toString();
        s.edgeColors[s.currentEdge.id] = const_Colors.ShortestPathColor;
    ...
    this.markAsProcessed = function() {
        ...
        if (s.pqueue.length() > 0) {
            ...
            this.poseQuestion("assets/question_dijkstra_end_"+LNG.getLanguage()+".json");
        };
    };
    ...
    var getPseudocodeLines = function(divCounter) {
        if (s.oldID == 0 || s.oldID == -1) return divCounter == 0;
        else if (s.oldID == 1) return divCounter == 1;
        else if (s.oldID == 2) return divCounter == 3;
        else if (s.oldID == 3) return divCounter == 4;
        else if (s.oldID == 4) return divCounter == 5;
        else if (s.oldID == 5) return divCounter == 6;
        else if (s.oldID == 6) return divCounter == 2;
        else if (s.oldID == 7) return divCounter == 7;
        else return divCounter == 7;
    }
    ...
    this.updateDescriptionAndPseudocode = function() {        
        if (s.id != 8) {
            ...
            changeTextDijkstra(s.oldID, "tf1");
        };
    ...
    this.poseQuestion = function(questionURL) {
        ...
        request.done(function(data) {
            ...
            $("#tf1_div_Frage").html(LNG.K('aufgabe1_status_1') + " " + questionStats.gestellt + " " + LNG.K('aufgabe1_status_2')+" " + questionStats.numQuestions);
            ...
            for (var i = 0; i < question.answers.length; i++) {
                ...
                if (i === question.correctAnswerIndex) {
                    ...
                    $("#" + idInput).click(function() {
                        ...
                        $("#tf1_div_Antworten").html("<h2>"+LNG.K('aufgabe1_status_3')+": " + ans + "</h2>");
    ...
    this.addQuestionTab = function() {
        var li = "<li id='tf1_li_FrageTab'><a href='#tf1_div_FrageTab'>"+LNG.K('aufgabe1_status_4')+"</a></li>", id = "tf1_div_FrageTab";
    ...
    this.showResults = function() {
        warnBeforeLeave = false;
        var li = "<li id='tf1_li_ErgebnisseTab'><a href='#tf1_div_ErgebnisseTab'>"+LNG.K('aufgabe1_status_5')+"</a></li>", id = "tf1_div_ErgebnisseTab";
        $("#tf1_div_statusTabs").find(".ui-tabs-nav").append(li);
        $("#tf1_div_statusTabs").append("<div id='" + id + "'></div>");
        $("#tf1_div_statusTabs").tabs("refresh");
        $("#tf1_div_statusTabs").tabs("option", "active", 2);
        if (questionStats.numQuestions == questionStats.correct) {
            $("#tf1_div_ErgebnisseTab").append("<h2>"+LNG.K('aufgabe1_status_6')+"</h2>");
            $("#tf1_div_ErgebnisseTab").append("<p>"+LNG.K('aufgabe1_status_7')+"</p>");
        } else {
            $("#tf1_div_ErgebnisseTab").append("<h2>"+LNG.K('aufgabe1_status_8')+"</h2>");
            $("#tf1_div_ErgebnisseTab").append("<p>"+LNG.K('aufgabe1_status_9')+": " + questionStats.numQuestions + "</p>");
            $("#tf1_div_ErgebnisseTab").append("<p>"+LNG.K('aufgabe1_status_10')+": " + questionStats.correct + "</p>");
            $("#tf1_div_ErgebnisseTab").append("<p>"+LNG.K('aufgabe1_status_11')+": " + questionStats.wrong + "</p>");
            $("#tf1_div_ErgebnisseTab").append('<button id="tf1_button_Retry">'+LNG.K('aufgabe1_status_12')+'</button>');
            $("#tf1_button_Retry").button().click(function() {
                that.refresh();
            });
        }
        $("#tf1_div_ErgebnisseTab").append('<button id="tf1_button_gotoFA2">'+LNG.K('aufgabe1_button_4')+'</button>');
